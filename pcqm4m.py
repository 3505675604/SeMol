...
class PCQM4Mv2Dataset(InMemoryDataset):
    def __init__(
        ...
        self.data, self.slices = torch.load(self.processed_paths[0],weights_only=False)    #修改加了, weights_only=False
    ...
    def get_idx_split(self):
        split_dict = replace_numpy_with_torchtensor(
            torch.load(osp.join(self.root, "split_dict.pt"), weights_only=False)  #修改过
        )
        return split_dict
