...
def make_dataframe(dataset, dataset_type, tasks, tasks_wanted):
    ...
    # 1️⃣ 检查 SMILES 合法性，过滤无效分子
    valid_smiles = []
    valid_labels = []

    for idx, smi in enumerate(df["ids"]):
        if isinstance(smi, str):
            try:
                mol = Chem.MolFromSmiles(smi)
                if mol is not None:
                    valid_smiles.append(Chem.MolToSmiles(mol, isomericSmiles=True))
                    valid_labels.append(df.loc[idx, tasks_wanted].values.tolist())
                else:
                    print(f"⚠️  Skipping invalid SMILES at index {idx}: {smi}")
            except Exception as e:
                print(f"⚠️  Error parsing SMILES at index {idx}: {smi} ({e})")
        else:
            print(f"⚠️  Non-string SMILES at index {idx}: {smi}")
    ...
    print(f"✅ Cleaned dataset: kept {len(clean_df)} valid molecules out of {len(df)}")
